...
class InvestRecordsRepository {
  ...
  ///
  Future<InvestRecord?> getInvestRecord({required Isar isar, required int id}) async {
  ...
  Future<List<InvestRecord>?> getInvestRecordList({required Isar isar}) async {
    final IsarCollection<InvestRecord> investRecordsCollection = getCollection(isar: isar);
    return investRecordsCollection.where().sortByDate().thenByInvestId().findAll();
  }
  ...
  ///
  Future<void> inputInvestRecordList({required Isar isar, required List<InvestRecord> investRecordList}) async {
    final IsarCollection<InvestRecord> investRecordsCollection = getCollection(isar: isar);
    await isar.writeTxn(() async => investRecordsCollection.putAll(investRecordList));
  }
  ...
  ///
  Future<void> updateInvestRecord({required Isar isar, required InvestRecord investRecord}) async {
    final IsarCollection<InvestRecord> investRecordsCollection = getCollection(isar: isar);
    await investRecordsCollection.put(investRecord);
  }

  ///
  Future<void> deleteInvestRecordList({required Isar isar, required List<InvestRecord>? investRecordList}) async {
    investRecordList?.forEach((InvestRecord element) => deleteInvestRecord(isar: isar, id: element.id));
  }
  ...
  ///
  Future<void> deleteAllInvestRecords({required Isar isar}) async {
    final IsarCollection<InvestRecord> investRecordsCollection = getCollection(isar: isar);
    await isar.writeTxn(() async => investRecordsCollection.clear());
  }
}
